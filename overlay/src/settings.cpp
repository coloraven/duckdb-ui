...
std::string GetAssetsPath(const ClientContext &context) {
  auto path =
      internal::GetSetting<std::string>(context, UI_ASSETS_PATH_SETTING_NAME);
  return ui::ExpandHomePath(path);
}

bool GetOfflineMode(const ClientContext &context) {
  return internal::GetSetting<bool>(context, UI_OFFLINE_SETTING_NAME);
}
} // namespace duckdb
