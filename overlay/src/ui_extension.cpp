...
static void LoadInternal(DatabaseInstance &instance) {
  ...
  // CreateDirectory is not recursive; create parents first on a fresh machine.
  ...
  fs.CreateDirectory(fs.ExpandPath("~/.duckdb/extension_data/ui/assets"));
...

  {
    ...
    config.AddExtensionOption(UI_LOCAL_HOST_SETTING_NAME,
                              "Local host/interface on which the UI server listens",
  ...
  {
    auto def = GetEnvOrDefault(UI_ASSETS_PATH_SETTING_NAME,
                               UI_ASSETS_PATH_SETTING_DEFAULT);
    config.AddExtensionOption(
        UI_ASSETS_PATH_SETTING_NAME,
        "Local directory used to serve and cache UI static assets",
        LogicalType::VARCHAR, Value(def));
  }

  {
    auto def = IsEnvEnabled(UI_OFFLINE_SETTING_NAME) || UI_OFFLINE_SETTING_DEFAULT;
    config.AddExtensionOption(
        UI_OFFLINE_SETTING_NAME,
        "Air-gap assets: never fetch missing files from ui_remote_url (HTTP 503). "
        "Default false = local-first, miss fills from remote and caches into "
        "ui_assets_path. Browser third-party calls are always CSP-blocked.",
        LogicalType::BOOLEAN, Value::BOOLEAN(def));
  }
