...
// Accept both http://127.0.0.1:PORT and http://localhost:PORT as the UI origin.
bool IsAllowedBrowserOrigin(const std::string &origin, const std::string &local_host,
                            uint16_t local_port, const std::string &local_url) {
  if (origin.empty()) {
    return false;
  }
  if (origin == local_url) {
  ...
  if (local_host == "127.0.0.1") {
    return origin == StringUtil::Format("http://localhost:%d", local_port);
  }
  if (local_host == "localhost") {
    return origin == StringUtil::Format("http://127.0.0.1:%d", local_port);
  }
  return false;
}

bool IsAllowedBrowserReferer(const std::string &referer, const std::string &local_host,
                             uint16_t local_port, const std::string &local_url) {
  if (referer.compare(0, local_url.size(), local_url) == 0) {
    return true;
  }
  if (local_host == "127.0.0.1") {
    auto alt = StringUtil::Format("http://localhost:%d", local_port);
    return referer.compare(0, alt.size(), alt) == 0;
  }
  if (local_host == "localhost") {
    auto alt = StringUtil::Format("http://127.0.0.1:%d", local_port);
    return referer.compare(0, alt.size(), alt) == 0;
  }
  return false;
}

// UI "Add database" builds ATTACH via quoteSqlString(path). If the user pastes a
// path already wrapped in "..." or '...', strip one layer so DuckDB opens the
// real filesystem path. Safe no-op for normal unquoted paths.
void NormalizeAttachPathQuotes(std::string &sql) {
  auto is_ws = [](char c) {
    return c == ' ' || c == '\t' || c == '\n' || c == '\r';
  };
  auto ieq = [](const std::string &s, size_t pos, const char *lit) {
  ...
  const size_t lit_end = i;
  if (content.size() < 2) {
    return;
  }
  const char a = content.front();
  const char b = content.back();
  if (!((a == '"' && b == '"') || (a == '\'' && b == '\''))) {
    return;
  }
  ...
  rebuilt.push_back('\'');
  sql.replace(lit_start, lit_end - lit_start + 1, rebuilt);
}
...
bool HttpServer::IsRunningOnMachine(ClientContext &context) {
  ...
  auto probe_url = StringUtil::Format("http://%s:%d", local_host, local_port);

  httplib::Client client(probe_url);
...
const HttpServer &HttpServer::Start(ClientContext &context, bool *was_started) {
  ...
  const auto assets_path = GetAssetsPath(context);
  const auto offline_mode = GetOfflineMode(context);
  ...
  server->DoStart(port, host, remote_url, assets_path, offline_mode,
                  std::move(http_params));
  return *server;
}
...
void HttpServer::DoStart(const uint16_t _local_port,
                         ...
                         const std::string &_assets_path, bool _offline_mode,
...
void HttpServer::HandleGetLocalToken(const httplib::Request &req,
  ...
  if (!IsAllowedBrowserReferer(referer, local_host, local_port, local_url)) {
    res.status = 401;
    return;
  }
...
// Always lock the browser to same-origin. Asset miss→remote→cache is done by
// *this* server (server-side), so CSP does not block fill-in from ui_remote_url.
// It does stop the SPA from hanging on MotherDuck / Datadog / Auth0 XHR.
void ApplyLocalShellCsp(httplib::Response &res) {
  res.set_header(
      ...
      "worker-src 'self' blob:; form-action 'self'; base-uri 'self'");
}
...
void HttpServer::HandleGet(const httplib::Request &req,
                           httplib::Response &res) {
  // Prefer locally mirrored/cached UI assets.
  if (ServeLocalAsset(assets_path, req.path, res)) {
    if (req.path == "/config") {
      res.set_header("X-DuckDB-Version", DuckDB::LibraryVersion());
      res.set_header("X-DuckDB-Platform", DuckDB::Platform());
      res.set_header("X-DuckDB-UI-Extension-Version", UI_EXTENSION_VERSION);
    }
    ApplyLocalShellCsp(res);
    return;
  }

  // ui_offline=true: air-gap — never contact ui_remote_url for missing assets.
  if (offline_mode) {
    // /version is required by the shell to select local HTTP mode.
    if (req.path == "/version") {
      res.status = 200;
      res.set_content("{\"origin\":\"host\",\"version\":\"offline\"}",
                      "application/json; charset=utf-8");
      res.set_header("X-DuckDB-UI-Asset-Source", "synthetic");
      ApplyLocalShellCsp(res);
      return;
    }
    res.status = 503;
    res.set_content("ui_offline=true, but asset '" + req.path +
                        "' was not found under '" + assets_path +
                        "'. Mirror/extract assets into ui_assets_path, or set "
                        "ui_offline=false to allow miss→remote→cache.",
                    "text/plain");
    return;
  }

  // Default: miss → fetch ui_remote_url → cache into ui_assets_path.
  ...
  if (!result) {
    // Fall back to SPA index if we at least have a local shell.
    if (ServeLocalAsset(assets_path, "/", res)) {
      ApplyLocalShellCsp(res);
      return;
    }
    // /version: synthesize so the shell still picks local HTTP mode.
    if (req.path == "/version") {
      res.status = 200;
      res.set_content("{\"origin\":\"host\",\"version\":\"offline\"}",
                      "application/json; charset=utf-8");
  ...
  res = result.value();
  res.set_header("X-DuckDB-UI-Asset-Source", "remote");
  // Cache the upstream body before fork-only HTML injection.
  MaybeCacheRemoteAsset(req.path, res);

  const auto content_type = res.get_header_value("Content-Type");
  if (content_type.find("text/html") != std::string::npos) {
    InjectForkThemeToggle(res.body);
  }
...
void HttpServer::MaybeCacheRemoteAsset(const std::string &request_path,
                                       const httplib::Response &remote_res) {
  if (assets_path.empty() || remote_res.status < 200 ||
      remote_res.status >= 300 || remote_res.body.empty()) {
    return;
  }
  const auto content_type = remote_res.get_header_value("Content-Type");
  if (!ShouldCacheRemoteAsset(request_path, content_type)) {
    return;
  }
  WriteLocalAsset(assets_path, request_path, remote_res.body);
}

void HttpServer::HandleInterrupt(const httplib::Request &req,
                                 httplib::Response &res) {
  auto origin = req.get_header_value("Origin");
  if (!IsAllowedBrowserOrigin(origin, local_host, local_port, local_url)) {
    res.status = 401;
    return;
  }
...
void HttpServer::DoHandleRun(const httplib::Request &req,
  ...
  NormalizeAttachPathQuotes(content);
