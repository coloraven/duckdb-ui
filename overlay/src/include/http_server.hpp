...
private:
  ...
  void DoStart(const uint16_t local_port, const std::string &local_host,
               const std::string &remote_url, const std::string &assets_path,
               bool offline_mode, unique_ptr<HTTPParams>);
  void DoStop();
  void Run();
  ...
  void MaybeCacheRemoteAsset(const std::string &request_path,
